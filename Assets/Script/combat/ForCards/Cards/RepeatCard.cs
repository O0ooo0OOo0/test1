...
public class RepeatCard : MonoBehaviour
{
    ...
    void Update()
    {
        if (isused == false && targetDetector.isUseObjectInside && useCardDraggable.isDragging == false && targetDetector.UseObject.CompareTag("Many") && recordAction.type != 0 && ap.value >= consume)
        {
            JudgeCanBeUse();
        }
    }
