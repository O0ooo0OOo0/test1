...
public class InteractionPromptUI : MonoBehaviour
{
    ...
    public void UpdatePosition(Vector3 worldPosition)
    {
        ...
        // 将世界坐标转换为屏幕坐标
        Vector3 screenPos = mainCamera.WorldToScreenPoint(worldPosition + promptOffset);
        ...
        bool success = RectTransformUtility.ScreenPointToLocalPointInRectangle(
            canvasRect,
            screenPos,
            parentCanvas.worldCamera,  // ← 关键：传入 Canvas 的摄像机（不是 null）
            out localPoint
        );

        if (success)
        {
            interactPrompt.anchoredPosition = localPoint;
        }
